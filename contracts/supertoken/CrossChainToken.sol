...
contract SuperToken is ERC20, Gauge, AccessControl, ISocketReceiver {
    ...
    function updateLimitParams(
        ...
        for (uint256 i; i < updates_.length; i++) {
            if (updates_[i].isMint) {
                ...
                _sendingLimitParams[updates_[i].connector].maxLimit = updates_[
                    i
                ].maxLimit;
                _sendingLimitParams[updates_[i].connector]
                    .ratePerSecond = updates_[i].ratePerSecond;
    ...
    function bridge(
        ...
        bytes32 returnedMessageId = IConnector(connector_).outbound{
            value: msg.value
        }(msgGasLimit_, abi.encode(receiver_, sendingAmount_, messageId));
