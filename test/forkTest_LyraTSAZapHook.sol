...
contract LyraTSAZapHookForkTest is Test {
    ...
    LyraTSAZapHook hook = LyraTSAZapHook(payable(0x55328b5036EB15DdCA2a91468F1C70Dcae29b7Ab));
    Controller public controller = Controller(0xbEc0B31bbfA62364EBF6e27454978E33c5d9F4eE);
...
//    }
    ...
    function testHookWithdrawsTSAsAtomically() external {
        ...
        vm.recordLogs();
        bytes memory calldataToSend = hex"";
        vm.prank(caller);
        connectorPlug.call(calldataToSend);
    ...
    function testSendsToFallbackAddressInTheCaseOfInvalidConnector() external {
        ...
        bytes memory calldataToSend = hex"";
        vm.prank(caller);
        connectorPlug.call(calldataToSend);
...
//        _updateHookContract();
        ...
        _updateLimits(LimitHook(0xfeF7a0bF88bBAA9409485C6Fe343A4135D5E799f), 0x5f675ab715BB9D712e4628E74c8e11B46867aCe3, 0, 0);
        // then set to something that will have 0 limit
        _updateLimits(LimitHook(0xfeF7a0bF88bBAA9409485C6Fe343A4135D5E799f), 0x5f675ab715BB9D712e4628E74c8e11B46867aCe3, 1, 0);
    ...
    function _updateHookContract() internal {
        // Update hook
        LyraTSAZapHook hook = new LyraTSAZapHook(0x000000A94C901AA5d4da1157B2Dd1c4c6b69815e, 0xbEc0B31bbfA62364EBF6e27454978E33c5d9F4eE, true);
        vm.startPrank(0x000000A94C901AA5d4da1157B2Dd1c4c6b69815e);

        (bool success, ) = 0xbEc0B31bbfA62364EBF6e27454978E33c5d9F4eE.call(abi.encodeWithSignature(
        ...
        poolIds_[0] = 300749528249665590178224313442040528409305273634097553067152835846309150720;
        hook.updateConnectorPoolId(connectors, poolIds_);

        vm.stopPrank();

        // update limit params and fast forward
        _updateLimits(hook, 0x8FF3f8bc7884fe59425F090d5ec6A570472DfF88, 100000000000000000000000000, 100000000000000000000000000);
        uint currentTime = block.timestamp;
        vm.warp(currentTime + 1);
    }

    function _updateLimits(LimitHook hook, address connector, uint limit, uint rate) internal {
        vm.startPrank(0x000000A94C901AA5d4da1157B2Dd1c4c6b69815e);
        UpdateLimitParams[] memory updates = new UpdateLimitParams[](2);
        updates[0] = UpdateLimitParams(
            true, connector, limit, rate
        );
        updates[1] = UpdateLimitParams(
            false, connector, limit, rate
        );
